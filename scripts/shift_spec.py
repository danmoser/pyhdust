...
"""Program that shifts a FITS spectrum based on an line-profile Gaussian
fit (optimized to work within a few tenths of km/s).
"""
...
try:
    ...
    warn("lmfit module not installed!!!")
...
parser.add_argument(
    "INPUT",
    help=(
        "String to retrive the spectrum(a) "
        'filename(s) (wildcards accepted wrapped with "")'
    ),
    type=str,
)
parser.add_argument(
...
group2.add_argument(
    ...
    dest="rm",
    help=(
        "If this flag is enabled, it restores the spectrum(a) to the "
        "original value (i.e., VELSHIFT = 0)"
    ),
    default=False,
)
group2.add_argument(
    ...
    action="store",
    dest="vs",
    help=("Force VELSHIFT to the non-zero VS value (in km/s) " "[default: None]"),
    type=float,
    default=0.0,
)
...
def apply_shift(imfits, vshift, wl=None):
    if wl is None:
        ...
        wl = (
            np.arange(len(imfits[0].data)) * imfits[0].header["CDELT1"]
    ...
    print(
        "# {0} updated with {1:.1f} km/s shift (accum. {2:.0f} km/s)".format(
    ...
    for fitsfile in glob(linput):
        ...
        if args.rm is True:
            ...
            try:
                flux = imfits[0].data
                wl = (
                    np.arange(len(flux)) * imfits[0].header["CDELT1"]
                    + imfits[0].header["CRVAL1"]
                )
