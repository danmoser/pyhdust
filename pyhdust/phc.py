...
def baricent_calc(x0, y0, x, y, z, fullrange=False):
    ...
    if _np.sum(_np.isnan([x1, x2, x3, x4])) == 0:
        ...
        for i in seq:
            ...
            if not inside:
                X, Y = (xs[i], ys[i])
                alpha = ((Y[1] - Y[2]) * (x0 - X[2]) + (X[2] - X[1]) * (y0 - Y[2])) / (
                    (Y[1] - Y[2]) * (X[0] - X[2]) + (X[2] - X[1]) * (Y[0] - Y[2])
                )
                beta = ((Y[2] - Y[0]) * (x0 - X[2]) + (X[0] - X[2]) * (y0 - Y[2])) / (
                    (Y[1] - Y[2]) * (X[0] - X[2]) + (X[2] - X[1]) * (Y[0] - Y[2])
                )
        ...
        a = _np.zeros(3)
        a[2] = _np.abs(
            0.5 * (X[0] * (Y[1] - y0) + X[1] * (y0 - Y[0]) + x0 * (Y[0] - Y[1]))
        )
        a[1] = _np.abs(
            0.5 * (X[0] * (Y[2] - y0) + X[2] * (y0 - Y[0]) + x0 * (Y[0] - Y[2]))
        )
        a[0] = _np.abs(
            0.5 * (X[1] * (Y[2] - y0) + X[2] * (y0 - Y[1]) + x0 * (Y[1] - Y[2]))
        )
...
def wg_avg_t(t, y, yerr=None):
    ...
    for i in range(n - 1):
        ...
        avg += (
            dx[i]
            * (y[i + 1] / yerr[i + 1] + y[i] / yerr[i])
            / (1.0 / yerr[i] + 1.0 / yerr[i + 1])
        )
...
def bindata(x, y, yerr=None, nbins=20, xlim=None, interp=False, perc=0, weightx=True):
    ...
    for i in range(nbins):
        selx = _np.where(abs(x - tmpx[i]) <= shift / 2.0 + 1e-6)
        if len(selx[0]) >= 1:
            if perc <= 0:
                if weightx:
...
def cart_rot(x, y, z, ang_xy=0.0, ang_yz=0.0, ang_zx=0.0):
    ...
    rotmtx = _np.array(
        [
            [
                _np.cos(ang_zx) * _np.cos(ang_xy),
                -_np.cos(ang_yz) * _np.sin(ang_xy)
                + _np.sin(ang_yz) * _np.sin(ang_zx) * _np.cos(ang_xy),
                _np.sin(ang_yz) * _np.sin(ang_xy)
                + _np.cos(ang_yz) * _np.sin(ang_zx) * _np.cos(ang_xy),
            ],
            [
                _np.cos(ang_zx) * _np.sin(ang_xy),
                _np.cos(ang_yz) * _np.cos(ang_xy)
                + _np.sin(ang_yz) * _np.sin(ang_zx) * _np.sin(ang_xy),
                -_np.sin(ang_yz) * _np.cos(ang_xy)
                + _np.cos(ang_yz)
                + _np.sin(ang_zx) * _np.sin(ang_xy),
            ],
            [
                -_np.sin(ang_zx),
                _np.sin(ang_yz) * _np.cos(ang_zx),
                _np.cos(ang_yz) * _np.cos(ang_zx),
            ],
        ]
    )
    vec = _np.array([x, y, z])
...
def find_neighbours(par, par_grid, ranges, silent=True):
    ...
    for i in range(len(par)):
        ...
        if not is_inside_ranges(par[i], ranges[i]):
            if not silent:
                print(
                    "[find_neighbours] Warning: parameter entry out of "
...
def readfixwd(fname, wlims, chklims=True):
    ...
    if chklims and wlims[-1] < _np.max(lsz) - 1:
        wlims += [_np.max(lsz)]
    out = _np.chararray(
        (len(lines), len(wlims) - 1),
        itemsize=_np.max([_np.max(_np.diff(wlims)), wlims[0]]),
    )
    for j in range(len(lines)):
        out[j] = [
            lines[j][wlims[i - 1] : wlims[i]].strip().replace(",", "^")
            for i in range(1, len(wlims))
        ]
    return out


def readtextable(fname, commentchars="#", splitchars=r"&|\pm", delchars=r"\$" + "\r "):
    lines = fileread(fname).split("\n")
    out = []
    for il in lines:
        if len(il) == 0:
...
def mag_plot(ax, ylabel=r"mag$_V$", civcfg=[1, "y"], civdt=[1996, 1, 1], civlabel="%Y"):
    """TODO"""
    # ax.legend()
...
def civil_ticks(
    ax, civcfg=[1, "m"], civdt=None, tklab=True, label="%y %b %d", **kwargs
):
...
def savefig(fig, figname=None, fmt=["png"], keeppt=False, dpi=80, transp=True):
    ...
    for f in fmt:
        fig.savefig(
            figname + ".{0}".format(f), transparent=transp, bbox_inches="tight", dpi=dpi
        )
...
def normGScale(val, vmin=None, vmax=None, log=False):
    ...
    if not log:
        ...
        val = (
            (_np.log(_np.array(val).astype(float)) - _np.log(vmin))
            / (_np.log(vmax) - _np.log(vmin))
            * 255
        )
...
def BBlbd(T, lbd=None):
    ...
    1e-8 #Angs -> cm"""
    if lbd is None:
        lbd = _np.arange(1000, 10000.0, 100) * 1e-8  # Angs -> cm
...
def fBBcor(T):
    ...
    OUTPUT: fBBcor(T) < 1.0"""
    return 1.015 - 0.301 * (T / 1e4) + 0.064 * (T / 1e4) ** 2.0
...
def gbf(T, lbd):
    ...
    return _np.exp(g0 + g1 * _np.log(lbd) + g2 * _np.log(lbd) ** 2), _np.exp(
        b0 + b1 * _np.log(lbd) + b2 * _np.log(lbd) ** 2
    )
...
alpha = Constant(7.2973525664e-3, 7.2973525664e-3, "", "Fine structure constant")
...
G = Constant(6.67384e-8, 6.67384e-11, "cm3 g-1 s-2", "Gravitational constant")
Mea = Constant(398600.4418e15 / G.cgs, 398600.4418e9 / G.SI, "g", "Earth mass")
Rea = Constant(6371.0e5, 6371.0e3, "cm", "Earth radius")
Mju = Constant(126686535e15 / G.cgs, 126686535e9 / G.SI, "g", "Jupiter mass")
Rju = Constant(71492.0e5, 71492.0e3, "cm", "Jupiter radius")
c = Constant(2.99792458e10, 299792458.0, "cm s-1", "speed of light in vacuum")
sigma = Constant(
    5.670400e-5, 5.670400e-8, "erg cm-2 K-4 s-1", "Stefan-Boltzmann constant"
)
...
pc = Constant(
    au.cgs * 60 * 60 * 180 / _np.pi, au.SI * 60 * 60 * 180 / _np.pi, "cm", "Parsec"
)
e = Constant(
    10 * c.cgs * 1.6021766208e-19, 1.6021766208e-19, "esu", "Elementary charge"
)
a = Constant(
    4 * sigma.cgs / c.cgs,
    4 * sigma.SI / c.SI,
...
hbar = Constant(
    h.cgs / 2 / _np.pi, h.SI / 2 / _np.pi, "erg s", "Planck constant/(2*pi)"
)
# From Wikipedia
