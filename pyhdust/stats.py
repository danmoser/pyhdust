...
def cdf(x, xlim=None, savefig=False):
    ...
    n = len(x)
    # probability
    p = _np.arange(n) / (n - 1.0)
    sortedx = _np.sort(x)
    if xlim is None:
        xlim = [sortedx[0], sortedx[-1]]
    linx = _np.linspace(xlim[0], xlim[1], n)
    madx = mad(x)
    gausx = _np.random.randn(n) * madx / 2.0 + (xlim[0] + xlim[1]) / 2.0
    ...
    ax.set_xlim(xlim)
    ax.legend(loc="best", fancybox=True, framealpha=0.5, fontsize=8, labelspacing=0.05)
    ax.set_ylabel("c.d.f.")
    ax.set_ylim([0, 1])
    if savefig:
...
def snr(count_rate, texp=1.0, nexp=1, npix=10.0, bg=10.0, dk=0.0, ron=2.0, var=0.0):
    ...
    return (
        ...
        * nexp
        / _np.sqrt(
            texp * nexp * (count_rate + bg * npix + dk * npix)
            + ron**2 * npix * nexp
            + var
        )
    )
...
def corr_coef_cov_with_err(x, y, yerr, xerr=None, clear_nan=True, nsample=1000):
    r"""TO BE DONE
