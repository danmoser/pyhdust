...
def doHDR(
    RGB, levels=6, folder="hdr", ext="png", strength=[1.0], naturalness=[1.0], size=None
):
    ...
    for i in range(levels):
        ...
        rgb2png(img, savename="{}/hdr_{}".format(folder, i), size=size, ext=ext)
    #
...
def do_background(
    ...
    out = aF[..., :3] * tF[..., _np.newaxis] + aB[..., :3] * tB[..., _np.newaxis]
...
def align_backfore(bkimg, frimg, pos=[0.1, 0.9]):
    """doc"""
    ...
    if len(_np.shape(frimg)) > 3:
        ...
        return None
    elif f0h == bkh and f0v == bkv and pos == [0.5, 0.5]:
        # Nothing to do!
        print("# Nothing done!!")
        return frimg
    # Starting...
