...
def geneva_closest(Mstar, oblat, t, Zstr='014', tar=None, plot=False, silent=True):
    '''
    ...
    code will take Zstr='014' by default. If plot=True, it plots the
    interpolation of the stellar parameters as a function of rotation
    rates.
    '''
    from stuff import oblat2w, r_roche
    ...
    # read tar file
    if tar == None:
        dir0 = '/home/vieira/Dropbox/python/defs/geneva_models/'
        fmod = 'Z{:}.tar.gz'.format(Zstr)
        tar = tarfile.open(dir0 + fmod, 'r:gz')
    else:
    ...
    if w <= wlist.max():
        ...
        if not silent:
            print('[geneva_closest] Warning: no model rotating this fast at this age, taking Re/Rp={:.2f} instead.'.format(r_roche(wlist.max(), 90.)))
    ...
    if plot:
        ...
        from stuff import custom_plot_font
        custom_plot_font(12)
        fig, ax = plt.subplots(nrows=1, ncols=3, sharex=False, sharey=False, figsize=(10, 3.5))
        plt.subplots_adjust(left=.07, bottom=.14, right=.97, top=.85, wspace=.35, hspace=None)
        ax[0].set_title(r'$M_{{\star}}/M_{{\odot}}={:},\quad '.format(Mlist[iM]) \
            + 'R_{{\mathrm{{eq}}}}/R_{{\mathrm{{p}}}}={:},\quad '.format(oblat) \
            + 't/t_{{\mathrm{{MS}}}}={:}$'.format(t), \
            horizontalalignment='left', fontsize=18)
        ax[0].set_xlabel('$\Omega/\Omega_{\mathrm{c}}$', fontsize=14)
        ax[0].set_ylabel('$R_{\mathrm{pole}}\,[\mathrm{R_{\odot}}]$', fontsize=14)
        ax[0].plot(wlist, Rplist, color='gray', marker='o')
        ax[0].plot(w, Rpole, 'ro', markersize=10)
        ax[1].set_xlabel('$\Omega/\Omega_{\mathrm{c}}$', fontsize=14)
        ax[1].set_ylabel('$\log(L/L_{\odot})$', fontsize=14)
        ax[1].plot(wlist, logLlist, color='gray', marker='o')
        ax[1].plot(w, logL, 'ro', markersize=10)
        ax[2].set_xlabel('$\Omega/\Omega_{\mathrm{c}}$', fontsize=14)
        ax[2].set_ylabel('$\mathrm{age}\,[\mathrm{Myr}]$', fontsize=14)
        ax[2].plot(wlist, agelist, color='gray', marker='o')
        ax[2].plot(w, age, 'ro', markersize=10)
        plt.savefig('M{:}_ob{:}_t{:}.png'.format(Mlist[iM], oblat, t), dpi=100, bbox_inches='tight')
        plt.close()

    return Rpole, logL, age

def geneva_interp(Mstar, oblat, t, Zstr='014', tar=None, plot=False, silent=True):
    '''
    Interpolates Geneva stellar models.

    Usage:
    Rpole, logL, age = geneva_interp(Mstar, oblat, t, tar=None, plot=False)
    ...
    rates.
    '''
    from stuff import oblat2w , geneva_closest
    import tarfile
    from scipy.interpolate import griddata
...
def geneva_interp_fast(Mstar, oblat, t, Zstr='014', silent=True):
    ...
    '''
    from stuff import find_neighbours
    from scipy.interpolate import griddata
    ...
    # set ranges
    ranges = np.array([[par_grid[:, i].min(), par_grid[:, i].max()] for i in xrange(len(par))])
...
def geneva_pre_computed(Zstr='014'):
    ...
    '''
    from stuff import geneva_interp
    import tarfile

    dir0 = '/home/vieira/Dropbox/python/defs/geneva_models/'

    try:
        data = np.load(dir0 + 'geneva_interp_Z{:}.npz'.format(Zstr))
    except:
        # par grid
        Mstar_arr = np.array([1.7, 2., 2.5, 3., 4., 5., 7., 9., 12., 15., 20.])
        oblat_arr = np.linspace(1., 1.5, 6)
        t_arr = np.hstack([np.linspace(0., .9, 10), np.linspace(1., 1.1, 21)])
        
        Rpole_grid = np.zeros([len(Mstar_arr), len(oblat_arr), len(t_arr)])
        logL_grid = np.zeros([len(Mstar_arr), len(oblat_arr), len(t_arr)])
        age_grid = np.zeros([len(Mstar_arr), len(oblat_arr), len(t_arr)])
    
        # read tar file
        tar = tarfile.open(dir0 + 'Z{:}.tar.gz'.format(Zstr), 'r:gz')
    
        for iM, Mstar in enumerate(Mstar_arr):
            for iob, oblat in enumerate(oblat_arr):
                for it, t in enumerate(t_arr):
                    print(Mstar, oblat, t)
                    Rp, lL, age = geneva_interp(Mstar, oblat, t, tar=tar, plot=False)
                    Rpole_grid[iM, iob, it] = Rp
                    logL_grid[iM, iob, it] = lL
                    age_grid[iM, iob, it] = age
        np.savez(dir0 + 'geneva_interp_Z{:}'.format(Zstr), Mstar_arr=Mstar_arr, oblat_arr=oblat_arr, t_arr=t_arr, \
                 Rpole_grid=Rpole_grid, logL_grid=logL_grid, age_grid=age_grid)

    # high M    
    try:
        data = np.load(dir0 + 'geneva_interp_Z{:}_highM.npz'.format(Zstr))
    except:
        # par grid
        Mstar_arr = np.array([20., 25., 32., 40., 60., 85., 120.])
        oblat_arr = np.linspace(1., 1.05633802817, 2)
        t_arr = np.hstack([np.linspace(0., .9, 10), np.linspace(1., 1.1, 21)])
        ...
        for iM, Mstar in enumerate(Mstar_arr):
            for iob, oblat in enumerate(oblat_arr):
                for it, t in enumerate(t_arr):
                    ...
                    logL_grid[iM, iob, it] = lL
                    age_grid[iM, iob, it] = age
        np.savez(dir0 + 'geneva_interp_Z{:}_highM'.format(Zstr), Mstar_arr=Mstar_arr, oblat_arr=oblat_arr, t_arr=t_arr, \
                 Rpole_grid=Rpole_grid, logL_grid=logL_grid, age_grid=age_grid)
