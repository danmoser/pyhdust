...
def lineProf(x, flx, lbc, flxerr=_np.empty(0), hwidth=1000.0, ssize=0.05):
    """
...
def ECcalc(vels, flux, ssize=0.05, gaussfit=False, doublegf=True):
    """
    Supoe que o fluxo jah estah normalizado, e vetores ordenados.
...
def PScalc(vels, flux, vc=0.0, ssize=0.05, gaussfit=False):
    """
...
def FWHM(vels, flux, halfmax, vmax=350.0, flxincr=0.01):
...
def DCcalc(vels, flux, vmax=None, vc=0.0, ssize=0.05):
    """
...
def analline(lbd, flux, lbdc, hwidth=1000, verb=True, gaussfit=False, doublegf=True):
    """
...
def cardelli(lbd, flux, ebv=0.0, Rv=3.1):
    """
...
def cutpastrefspec(ivl, iflx, irefvl, ireflx, hwidth, ssize=0.05):
...
def normcontinuum_std(flux, ssize=0.05):
    """
