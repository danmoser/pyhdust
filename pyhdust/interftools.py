...
def readmap(mapfile, quiet=False, old=False):
    ...
    if mapfile.find(".maps") > 0:
        ...
        upck = ">{}f".format(npxs)
        data = _np.array(_struct.unpack(upck, f[ixdr : ixdr + npxs * 4])).reshape(
            (nm, nobs, lnum, ny, nx)
        )
        ixdr += npxs * 4
    elif mapfile.find(".map") > 0:
        ...
        for i in range(dfact):
            ...
            tmp[i * npxs : (i + 1) * npxs] = _np.array(
                _struct.unpack(upck, f[ixdr : ixdr + npxs * 4])
            )
            ixdr += npxs * 4
    ...
    lbdc = _np.zeros(lnum)
    for i in range(lnum):
        lbdc[i] = (lbdarr[i] + lbdarr[i + 1]) / 2.0

    return data, obslist, lbdc, Ra, xmax


def savemap(mfile, imap, obslist, lbdc, Ra, xmax, new_pref="BIN_", logspaced=False):
    r"""
    Read *Hdust* MAP or MAPS files.
...
def bin_map(mfile, nbins, new_pref="BIN_"):
    ...
    for ii, io, iy, ix, ic in _iprod(
        range(nimgs), range(nobs), range(ny), range(nx), range(dfact)
    ):

        if dfact == 6:
            outmap[ii, io, :, iy, ix, ic] = _phc.bindata(
                range(int(nlbd)), imap[ii, io, :, iy, ix, ic], nbins=nbins, interp=True
            )[1]
        if dfact == 1:
            outmap[ii, io, :, iy, ix] = _phc.bindata(
                range(int(nlbd)), imap[ii, io, :, iy, ix], nbins=nbins, interp=True
            )[1]
...
def img2fits(
    ...
    if rot != 0.0:
        ...
        img = _img.rotate_image(img, rot, 0, 0, fill=0.0)
...
def data2fitscube(
    ...
    hdulist[0].header["CRPIX1"] = len(imgs[1]) / 2.0
    hdulist[0].header["CRPIX2"] = len(imgs[2]) / 2.0
...
def setspacecoords(nx, ny, rad_per_pixel, xc=0.0, yc=0.0):
    """
    ...
    """
    x = _np.arange(0.0, nx) - (nx - 1) / 2.0 + xc
    xx = _np.repeat(x, ny).reshape(-1, ny).T * rad_per_pixel
    y = _np.arange(0.0, ny) - (ny - 1) / 2.0 + yc
    yy = _np.repeat(y, nx).reshape(-1, nx) * rad_per_pixel
    return xx, yy


def fastnumvis(img, lbd, Bproj, PA, rad_per_pixel, PAdisk=90.0, silent=False):
    """
    ...
    if (lbd < 1e-6 or lbd > 4e-6) and not silent:
        print("# Warning! *fastnumvis*(lbd) is out of JHK range {0:.1e} m".format(lbd))
    PA = PA - PAdisk + 90.0
    idx = _np.where(img > 0)

    u = Bproj * _np.double(_np.sin(PA / _np.double(180.0 / _np.pi)) / lbd)
    v = Bproj * _np.double(_np.cos(PA / _np.double(180.0 / _np.pi)) / lbd)
    # print PA,phc.ra2deg,lbd,Bproj,v

    ny = len(img)
    nx = len(img[0])
    xx, yy = setspacecoords(nx, ny, rad_per_pixel, xc=0.0, yc=0.0)
    ...
    VisPhase = _np.arctan2(complexVis.imag, complexVis.real) * _np.double(
        180.0 / _np.pi
    )
    return complexVis, VisAmp, VisPhase


def fastnumvis3(img, lbd, Bprojs, PAs, rad_per_pixel, PAdisk=90.0):
    ...
    """
    u1 = Bprojs[0] * _np.sin(PAs[0] * _np.pi / 180.0)
    u2 = Bprojs[1] * _np.sin(PAs[1] * _np.pi / 180.0)
    v1 = Bprojs[0] * _np.cos(PAs[0] * _np.pi / 180.0)
    v2 = Bprojs[1] * _np.cos(PAs[1] * _np.pi / 180.0)
...
class oiClass(object):
    ...
    def readvis2(self, oidata_vis2):
        ...
        for v2i in oidata_vis2:
            ...
            if v2i.station[0] and v2i.station[1]:
                tlist.append([v2i.station[0].sta_name + v2i.station[1].sta_name] * n)
            else:
    ...
    def readt3(self, oidata_t3):
        ...
        for t3i in oidata_t3:
            ...
            Pm = _np.arctan2(u2 + u1, v2 + v1) * 180.0 / _np.pi
            for uv in uvs:
                if _np.sqrt(uv[0] ** 2 + uv[1] ** 2) > Bm:
                    Bm = _np.sqrt(uv[0] ** 2 + uv[1] ** 2)
                    Pm = _np.arctan2(uv[0], uv[1]) * 180.0 / _np.pi
    ...
    def readvis(self, oidata_vis):
        ...
        for vi in oidata_vis:
            ...
            if vi.station[0] and vi.station[1]:
                tlist.append([vi.station[0].sta_name + vi.station[1].sta_name] * n)
            else:
                tlist.append(["unnamed"] * n)
    ...
    def add_model(self, imgs, rpx, PAdisk=90, modid=None, lbm=None):
        ...
        if self.oitype.upper().startswith("PIO"):
            ...
            for i in range(len(self.v2B)):
                if iscube:
                    img = imgs[_phc.find_nearest(self.v2lbm, self.v2lb[i], idx=True)]
                _, v, _ = fastnumvis(
                    img, self.v2lb[i], self.v2B[i], self.v2PA[i], rpx, PAdisk=PAdisk
                )
                tlist.extend([v**2])
            ...
            for i in range(len(self.t3B1)):
                if iscube:
                    img = imgs[_phc.find_nearest(self.t3lbm, self.t3lb[i], idx=True)]
                _, _, p = fastnumvis3(
                    img,
                    self.t3lb[i],
                    [self.t3B1[i], self.t3B2[i]],
                    [self.t3PA1[i], self.t3PA2[i]],
                    rpx,
                    PAdisk=PAdisk,
                )
                tlist.extend([p])
        ...
        #
            ...
            for i in range(len(self.vB)):
                if iscube:
                    img = imgs[_phc.find_nearest(self.vlbm, self.vlb[i], idx=True)]
                _, v, vp = fastnumvis(
                    img, self.vlb[i], self.vB[i], self.vPA[i], rpx, PAdisk=PAdisk
                )
                tlist_v.extend([v])
                tlist_vp.extend([vp])
...
def plot_pio_res(
    ...
    gs0.update(wspace=4.0)
    gs00 = _gridspec.GridSpecFromSubplotSpec(
        3, 2, subplot_spec=gs0[0, 0 : cols - 2], wspace=0.1, hspace=0.01
    )
    gs01 = _gridspec.GridSpecFromSubplotSpec(
        2, 1, subplot_spec=gs0[0, cols - 2 :], hspace=1.0
    )
    ...
    for i in range(len(loic)):
        ...
        if i == 0:
            axB1 = uv_plot_ax(
                axB1, oic, PAs=PAs, legend=legend, lbd=False, legbbox=legbbox, ard=ard
            )
            axB2 = uv_plot_ax(axB2, oic, PAs=PAs, legend=False, lbd=True, ard=ard)
    ...
    if v2leg is not None:
        ...
        for i in range(len(loic)):
            lines.extend(
                [_Line2D([], [], color=lcor[i], marker=lmk[i], ls="", label=v2leg[i])]
            )
...
def plot_pionier(
    ...
    if lmk is None:
        lmk = [_phc.cycles(i, "mk") for i in range(len(loic))]
    lins, cols = (2, 5)
    fig = _plt.figure(figsize=3.0 * _np.array([cols, lins]))
    gs = _gridspec.GridSpec(
        lins, cols, width_ratios=[1] * lins, height_ratios=[1] * cols
    )
...
def t3res_plot_ax(ax, loic, PAs=[-180, 180], lcor=None, lmk=None, rylim=None):
    ...
    #
    idx = _np.where(
        ((loic[0].t3Pm >= PAs[0]) & (loic[0].t3Pm <= PAs[1]))
        | ((loic[0].t3Pm >= PArv[0]) & (loic[0].t3Pm <= PArv[1]))
    )
    for j, oic in enumerate(loic[1:], 1):
        ax.scatter(
            loic[0].t3Bm / oic.t3lb[idx],
            (loic[0].t3p[idx] - oic.t3p[idx]) / (loic[0].t3pe[idx]),
            marker=lmk[j],
            alpha=0.7,
            color=lcor[j],
        )
...
def v2res_plot_ax(ax, loic, PAs=[-180, 180], lcor=None, lmk=None, rylim=None):
    ...
    #
    idx = _np.where(
        ((loic[0].v2PA >= PAs[0]) & (loic[0].v2PA <= PAs[1]))
        | ((loic[0].v2PA >= PArv[0]) & (loic[0].v2PA <= PArv[1]))
    )
    for j, oic in enumerate(loic[1:], 1):
        ax.scatter(
            loic[0].v2B / oic.v2lb[idx],
            (loic[0].v2[idx] - oic.v2[idx]) / (loic[0].v2e[idx]),
            marker=lmk[j],
            alpha=0.7,
            color=lcor[j],
        )
...
def t3_plot_ax(ax, oic, PAs=[-180, 180], cor="k", mk="o"):
    ...
    #
    idx = _np.where(
        ((oic.t3Pm >= PAs[0]) & (oic.t3Pm <= PAs[1]))
        | ((oic.t3Pm >= PArv[0]) & (oic.t3Pm <= PArv[1]))
    )
    ax.errorbar(
        oic.t3Bm / oic.t3lb[idx],
        oic.t3p[idx],
        yerr=oic.t3pe[idx],
        ls="",
        marker=mk,
...
def v2_plot_ax(ax, oic, PAs=[-180, 180], cor="k", mk="o"):
    ...
    #
    idx = _np.where(
        ((oic.v2PA >= PAs[0]) & (oic.v2PA <= PAs[1]))
        | ((oic.v2PA >= PArv[0]) & (oic.v2PA <= PArv[1]))
    )
    ax.errorbar(
        oic.v2B / oic.v2lb[idx],
        oic.v2[idx],
        yerr=oic.v2e[idx],
        ls="",
        marker=mk,
...
def uv_plot_ax(
    ax, oic, PAs=[-180, 180], legend=True, lbd=False, legbbox=None, ard=None
):
    ...
    #
    idx = _np.where(
        ((PA >= PAs[0]) & (PA <= PAs[1])) | ((PA >= PArv[0]) & (PA <= PArv[1]))
    )
    #
    u = B * _np.sin(PA * _np.pi / 180.0) / lbds
    v = B * _np.cos(PA * _np.pi / 180.0) / lbds
    ax.scatter(u[idx], v[idx], color=colors[idx], alpha=0.7, zorder=10)
    ax.scatter(-u[idx], -v[idx], color=colors[idx], alpha=0.7, zorder=10)
    if legend:
        ...
        for i in range(len(oic.v2Bavg)):
            lines.extend(
                [_Line2D(range(1), range(1), color=colors[i], marker="o", ls="")]
            )
    ...
    if ard is not None:
        ...
        ax.arrow(
            ...
            head_width=0.1 * _np.max(B) / _np.max(lbds),
            fc="gray",
            ec="gray",
            head_length=0.15 * _np.max(B) / _np.max(lbds),
            zorder=1,
        )
...
def plot_baseline(
    ...
    for vis2 in oidata.vis2:
        ...
        # [u,-u] = W > E
        # [-u,u] = E < W
        if (PA >= PAs[0] and PA <= PAs[1]) or (
            PAsrev and (PA >= PArv[0]) and PA <= PArv[1]
        ):
            if label not in leg:
...
# Other?
def mapinterf(
    modf, im=0, obs=0, iflx=0, dist=10, PA=0.0, B=100.0, PAdisk=90.0, quiet=False
):
    ...
    for i in range(npts):
        ...
        tmp, V, DP[i] = fastnumvis(
            img, lbdc[i] * 1e-6, B, PA, rad_per_pixel, PAdisk=PAdisk
        )
        V2[i] = V**2
    # avg = (DP[0]+DP[-1])/2.
    ssize = int(0.05 * len(DP))
    if ssize == 0:
        ssize = 1
...
def plot_v2_img(ax, oidata, img, xlim=None, quiet=False, alp=0.75):
    """IMG from LitPRO
    ...
    for i in range(len(x)):
        tmp, V, ph = fastnumvis(
            data, lbd[i], Blist[i], PAlist[i], rad_per_pixel, PAdisk=90.0
        )
        V2.append(V**2)
...
def plot_t3_img(ax, oidata, img, xlim=None, quiet=False, alp=0.75):
    """IMG from LitPRO
    ...
    for t3 in oidata.t3:
        ...
        PA1, PA2 = (
            _np.arctan2(u1, v1) * 180.0 / _np.pi,
            _np.arctan2(u2, v2) * 180.0 / _np.pi,
        )
        PAlist.extend([[PA1, PA2] for i in range(lbsz)])
        ax.errorbar(x, y, yerr=yerr, color="k", fmt="o", markersize=ms, alpha=alp)
    # #
    fits = _pyfits.open(img)
    dx = fits[0].header["CDELT1"]
    dy = fits[0].header["CDELT2"]
    ...
    t3 = []
    for i in range(len(x)):
        tmp, V, ph = fastnumvis3(
            data, lblist[i], Blist[i], PAlist[i], rad_per_pixel, PAdisk=90.0
        )
        t3.append(ph)
...
def plot_v2_res(
    ...
    for mod in modfiles:
        ...
        for deg in obs:
            ...
            if _np.abs(obslist[::2][ob] - deg) > 1:
                ...
                print(
                    "# Input: {0:.1f}; Nearest model: {1:.1f}".format(
                        deg, obslist[::2][ob]
                    )
                )
            for i in range(len(x)):
                j = list(lbdc * 1e-6).index(_phc.find_nearest(lbdc * 1e-6, lblist[i]))
            ...
            _phc.savefig(fig2, figname="{0}_{1}".format(_os.path.basename(mod), ob))
            V2 = _np.array(V2)
            if bindata <= 0:
                ax.plot(
                    x,
            ...
            if printsum:
                ...
                print(v2sum)
                ax2.text(
                    0.4, 0.87, r"$\sum={0:.1f}$".format(v2sum), transform=ax2.transAxes
                )
    # Blim = ax3.get_xlim()
    # ax4.set_xlim(Blim)
    ...
    ax2.set_ylabel("$V$ $^2$(data-mod)/err")
    if xlim is not None:
        ax2.set_xlim(xlim)
...
def plot_phi3_res(
    ...
    for t3 in oidata.t3:
        ...
        # PA = _np.arctan2(u, v) * 180.0 / _np.pi
        Bcompar = _np.sqrt(
            [u1**2 + v1**2, u2**2 + v2**2, (u1 + u2) ** 2 + (v1 + v2) ** 2]
        )
        PAcompar = (
            _np.array(
                [
        ...
        if (PA[0] >= PAs[0] and PA[0] <= PAs[1]) or (
            PAsrev and PA[0] >= PArv[0] and PA[0] <= PArv[1]
        ):
            Bmax.extend(B)
            lbsz = len(t3.wavelength.eff_wave)
            x.extend(B / t3.wavelength.eff_wave)
    ...
    if bindata <= 0:
        ax.errorbar(
            x, y, yerr=yerr, color=color, fmt="o", markersize=ms, zorder=1, alpha=alp
        )
    else:
        binned = _phc.bindata(
            B / t3.wavelength.eff_wave, t3.t3phi, bindata, yerr=t3.t3phierr, xlim=xlim
        )
        ax.errorbar(
            binned[0],
            binned[1],
    ...
    for mod in modfiles:
        ...
        for deg in obs:
            ...
            if bindata <= 0:
                ...
                binned = _phc.bindata(Bmax / lblist, t3m, bindata, xlim=xlim)
                ax.plot(binned[0], binned[1], color=mcolors[k], alpha=0.9)
                binned = _phc.bindata(Bmax / lblist, (y - t3m) / yerr, bindata)
                ax2.plot(
                    binned[0],
                    binned[1],
                    color=mcolors[k],
...
class Disk(object):
    ...
    def __init__(
        self, lbd=[0.001], barz2=1.0, Td=12120.0, fion=1.0, mu=0.5, gb=None, rho=2.8e-11
    ):
    ...
    def update(self):
        ...
        self.kappa = (
            3.692e8
            * (1 - _np.exp(-_phc.h.cgs * _phc.c.cgs / self.lbd / _phc.kB.cgs / self.Td))
            * self.barz2
            * self.Td**-0.5
            * (self.lbd / _phc.c.cgs) ** 3
            * self.fion
            * (self.rho / self.mu / _phc.mH.cgs) ** 2
            * (self.gb)
        )
...
def I(
    ...
    print(
        _tab.tabulate(
            _np.array([lbd, Disk.kappa, tau0, barR]).T,
...
def plot_pio_res_old(
    ...
    colors = _phc.gradColor(
        Blist, min=Blist[0] - (Blist[-1] - Blist[0]) * 0.1, cmapn="plasma_r"
    )
...
def plot_uv_old(
    ax, oidata, colors, names, PAs=[-180, 180], PAsrev=False, PArv=[0, 0], xlim=None
):
    ...
    for vis2 in oidata.vis2:
        ...
        if (PA >= PAs[0] and PA <= PAs[1]) or (
            ...
            ax.plot([ulbd, -ulbd], [vlbd, -vlbd], ".", color=colors[names.index(label)])
    f = 1.4 * 5e7 / 112.0
    x = 84.06 * f
    y = 111.96 * f
    ax.arrow(
...
def plot_pionier_old(
    oidata, ffile="last_run", fmt=["png"], legend=True, model=None, obs=None, dist=None
):
    ...
    for vis2 in oidata.vis2:
        ...
        line = ax4.errorbar(
            PAobs, vis2.vis2data, yerr=vis2.vis2err, color=color, fmt="o", markersize=ms
        )
        # , label=label)
    Blim = ax3.get_xlim()
    if model is not None:
        ...
        for mod in model:
            ...
            for vis2 in oidata.vis2:
                ...
                # alpha=.3)
                ax3.plot(B / lbds, V2, color="purple", alpha=0.3)
                ax4.plot(
                    _np.tile(PA, len(lbds)),
                    V2,
                    color="purple",
                    alpha=0.3,
                    marker="s",
    ...
    for t3 in oidata.t3:
        ...
        # , label=label)
        line = ax6.errorbar(PA, y, yerr=yerr, color=color, fmt="o", markersize=ms)
    if model is not None:
        for mod in model:
            if obs is None:
                obs = [0]
            ...
            for t3 in oidata.t3:
                ...
                ax5.plot(Bmax / lbds, t3m, color="purple", alpha=0.9)
                ax6.plot(
                    PAmax, t3m, color="purple", alpha=0.6, marker="s", markersize=ms
                )
    # ax5.get_xaxis().set_ticklabels([])
    ax5.set_xlim(Blim)
    ax6.set_xlim(PAlim)
    ...
    _plt.subplots_adjust(
        left=0.12, right=0.95, top=0.96, bottom=0.09, hspace=0.009, wspace=0.32
    )
...
def printinfo(file, extract=False):
    ...
    if extract:
        ...
        return (
            [info[2][:10], "{0:.7f}".format(info[1]), info[0]] + info2,
            wav,
            info3,
            info4,
        )
        ...
        for vis in oidata.vis:
            ...
            info2 += [
                "{0:.1f}".format(
                    _np.arctan2(vis.ucoord, vis.vcoord) * 180.0 / _np.pi % 180.0
                )
            ]
...
def plot_oifits(
    oidata, ffile="last_run", fmt=["png"], xlim=None, legend=True, outfold="./"
):
    ...
    if ffile.find("_PRO/PRODUCT") > 0:
        ...
        specfile = ffile[: ffile.find("_PRO/PRODUCT_")].replace(
            "_SPEC", ""
        ...
        if len(specfile) != 1:
            specfile = ""
            print("# ERROR! This is a PRO oifits and the AVG file was " + "not found!")
            print(
                ffile[: ffile.find("_PRO/PRODUCT_")]
                + ("/*{0}*_OIDATA_" + "AVG.fits*").format(dateobs)
            )
        else:
            specfile = specfile[0]
            ...
            if len(spec.wavelength.eff_wave) == len(vis.wavelength.eff_wave):
                ...
                print(
                    "# ERROR! spec and vis sizes are different for {0}".format(
                        _phc.trimpathname(ffile)[1]
                    )
                )
    #
    fig = _plt.figure(figsize=(5.6, 8))
    ...
    hdrinfo = oidata.hdrinfo.returninfo()
    for i in range(4):
        ax0.text(0.0, 0.8 - 0.2 * i, hdrinfo[i])
    # ax2 = uvplane.
    ...
    for vis in oidata.vis:
        if xlim is None:
            xmin = None
            xmax = None
            xmin = _np.min(vis.wavelength.eff_wave[_np.where(vis.flag is False)]) * 1e6
            xmax = _np.max(vis.wavelength.eff_wave[_np.where(vis.flag is False)]) * 1e6
            xlim = (xmin, xmax)
        u = vis.ucoord
        v = vis.vcoord
    ...
    for t3 in oidata.t3:
        ...
        if t3.station[0] and t3.station[1] and t3.station[2]:
            label = (
                t3.station[0].sta_name + t3.station[1].sta_name + t3.station[2].sta_name
            )
        else:
            label = "unnamed"
        color = colors[colorid]
        line = ax5.errorbar(
            1e6 * t3.wavelength.eff_wave,
            t3.t3phi,
            t3.t3phierr,
            label=label,
            color=color,
    ...
    for vis in oidata.vis2:
        yrange = [
            _np.min([yrange[0], _np.min(vis.vis2data)]),
    ...
    for suf in fmt:
        _plt.savefig("{2}/{0}.{1}".format(name, suf, outfold), transparent=True)
    _plt.close()
    return


def plot_pionier_res(
    oidata,
    model,
    outname=None,
    fmt=["png"],
    ...
    for vis2 in oidata.vis2:
        ...
        # [-u,u] = E < W
        # label=label,
        if (PA >= PArange[0] and PA <= PArange[1]) or (
            PArr and (PA >= PArv[0]) and PA <= PArv[1]
        ):
            ax1.plot([ulbd, -ulbd], [vlbd, -vlbd], ".", color=color)
    ax1.get_xaxis().set_ticklabels([])
    # ax1.xaxis.tick_top()
    ...
    for vis2 in oidata.vis2:
        ...
        for mod in model:
            ...
            V2 = _np.array(V2) + (0.0875e-8 * B / lbds - 0.026)
            if (PA >= PArange[0] and PA <= PArange[1]) or (
                PArr and (PA >= PArv[0]) and PA <= PArv[1]
            ):
                if bindata <= 0:
                    v2sum += (vis2.vis2data - V2) / vis2.vis2err
                    ax3.plot(B / lbds, V2, color=mcolors[k], alpha=0.3)
                    ax4.plot(
                        B / lbds,
                        (vis2.vis2data - V2) / vis2.vis2err,
                        color=mcolors[k],
                        markersize=ms,
                        marker="o",
                        ls="",
                    ...
                    ax3.plot(binned[0], binned[1], color=mcolors[k], alpha=0.3)
                    binned = _phc.bindata(
                        B / lbds,
                        (vis2.vis2data - V2) / vis2.vis2err,
                        bindata,
    ...
    ax4.set_ylim([-6, 6])
    ax4.get_xaxis().set_ticklabels([])
    ax4.set_ylabel("$V$ $^2$(data-mod)/err")
    if xlim is not None:
        ax4.set_xlim(xlim)
    ax4.grid(b=True, linestyle=":", alpha=alp)
    if shv2sum:
    ...
    for t3 in oidata.t3:
        ...
        y = t3.t3phi
        yerr = t3.t3phierr
        if (PA[0] >= PArange[0] and PA[0] <= PArange[1]) or (
            PArr and PA[0] >= PArv[0] and PA[0] <= PArv[1]
        ):
            if bindata <= 0:
                line = ax5.errorbar(
                    B / t3.wavelength.eff_wave,
                ...
                binned = _phc.bindata(
                    B / t3.wavelength.eff_wave, y, bindata, yerr=yerr, xlim=xlim
                )
                line = ax5.errorbar(
                    binned[0],
                    binned[1],
        ...
        for mod in model:
            ...
            if (PA[0] >= PArange[0] and PA[0] <= PArange[1]) or (
                ...
                if bindata <= 0:
                    ax5.plot(Bmax / lbds, t3m, color=mcolors[k], alpha=0.9)
                    ax6.plot(
                        Bmax / lbds,
                        (y - t3m) / yerr,
                        color=mcolors[k],
                    ...
                    ax5.plot(binned[0], binned[1], color=mcolors[k], alpha=0.9)
